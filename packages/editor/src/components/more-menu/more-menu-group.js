...
function toMenuItems( fills ) {
	return Children.map( fills, ( fill ) => {
		...
		// The fill renders the content of the item, so the label element the
		// item points at with `aria-labelledby` is never rendered. Naming falls
		// back to the content of the fill.
